...
def order_vars( Dam, N_holiday, G_holiday, W_n, W_b, W_c, Eco_outflow, data_type ):
    ...
    for i in range(data_type[2]+1 , comp - end):
        ...
        w = Normalize_data(Dam[5][i-1], Max[4], Min[4])
        y = Normalize_data(Dam[4][i-1], Max[3], Min[3]) 
        list_in_sec.append(y)
        list_in_sec.append(w) 
...
def clean_nan(Input, Output, Time_plot, Input_2, Output_2 ):
    ...
    if Y == 1:
        ...
        for i in range(X):
            ...
            if np.isnan(Output_2[i,0]):

                Output_2[i,0] = Output_2[i-1,0]
            ...
            for j in range(Input_2.shape[1]):
                
                if np.isnan(Input_2[i,j]):
                    
                    Input_2[i,j] = Input_2[i-1,j]
