...
def build_MLP(input_var, output_var, time_plot, data_index, hidden_layers_info, opt, test_parameters, name_model, save_model, data_type):
    ...
    if output_shape[1] > 2:   
        ...
        r_ = calculate_correlation(Test_out_pred[:,3], Test_out[:,3])
        r.append(r_)
        MAE_ = np.mean(abs(Test_out[:,3] - Test_out_pred[:,3]))
        MAE.append(MAE_)
    ...
    if save_model == 1:
        ...
        if data_type[1] == 1: 
            ...
            print(Test_out_pred[:,0].shape)
            make_plot(path, 'Model_2_storage', time_scale, 'Date', 'Storage(m)', Test_out[:,0], 'Real Data', Test_out_pred[:,0], 'Estimation Storage')
            ...
            make_plot(path, 'Model_2_storage', time_scale, 'Date', 'Storage(Hm^3)', Test_out[:,0], 'Real Data', Test_out_pred[:,0], 'Estimation Storage')
        
        if output_shape[1] == 2:
            Test_out[:,1] = Denormalize_data(Test_out[:,1], 250, 0)
            Test_out_pred[:,1] = Denormalize_data(Test_out_pred[:,1], 250, 0) 
            make_plot(path, 'Model_2_outflow', time_scale, 'Date', 'Outflow (m^3/s)', Test_out[:,1], 'Real Data', Test_out_pred[:,1], 'Estimation Total Outflow')
        else:
            Test_out[:,1] = Denormalize_data(Test_out[:,1], 10, 0)
            Test_out_pred[:,1] = Denormalize_data(Test_out_pred[:,1], 10, 0) 
            make_plot(path, 'Model_2_Bottom', time_scale, 'Date', 'Outflow (m^3/s)', Test_out[:,1], 'Real Data', Test_out_pred[:,1], 'Estimation Outflow Flood')
            Test_out[:,2] = Denormalize_data(Test_out[:,2], 130, 0)
            Test_out_pred[:,2] = Denormalize_data(Test_out_pred[:,2], 130, 0) 
            make_plot(path, 'Model_2_Flood', time_scale, 'Date', 'Outflow (m^3/s)', Test_out[:,2], 'Real Data', Test_out_pred[:,2], 'Estimation Outflow Bottom')
            Test_out[:,3] = Denormalize_data(Test_out[:,3], 130, 0)
            Test_out_pred[:,3] = Denormalize_data(Test_out_pred[:,3], 130, 0) 
            make_plot(path, 'Model_2_Power', time_scale, 'Date', 'Outflow (m^3/s)', Test_out[:,3], 'Real Data', Test_out_pred[:,3], 'Estimation Outflow Power')
