...
def build_MLP_sec(input_var, output_var, time_plot, data_index, hidden_layers_info, opt, test_parameters, name_model, data_type, show_progress ):
    ...
    if data_type[1] == 1: 
        ...
        make_plot_ns(path, 'Model_3_sec_storage', time_scale, 'Date', 'Storage(m)', Test_out[:,0], 'Real Data', Test_out_pred[:,0], 'Estimation Storage')
        
    elif data_type[1] == 2:
        
        Test_out[:,0] = Denormalize_data(Test_out[:,0], 320, 80)
        Test_out_pred[:,0] = Denormalize_data(Test_out_pred[:,0], 320, 80)
        make_plot_ns(path, 'Model_3_sec_storage', time_scale, 'Date', 'Storage(Hm^3)', Test_out[:,0], 'Real Data', Test_out_pred[:,0], 'Estimation Storage')
    ...
    if save_model == 1:
        ...
        # Plot loss while training 
        if data_index[1]==data_index[0]:
            make_plot(path, 'Model_3_training_sec', Epochs_axis, 'Epochs', 'Loss (mse)', history.history['loss'])
        else:
            make_plot(path, 'Model_3_training_sec', Epochs_axis, 'Epochs', 'Loss (mse)', history.history['loss'], 'Training', history.history['val_loss'], 'Validation')
