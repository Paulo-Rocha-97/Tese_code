...
# This function only averages Met_1, Met_2, Met_3 and Met_4 

def averaging_lists(Var_1, Var_2, Var_3, Var_4):

    Met_ave = []
    
    if len(Var_1[0]) == len(Var_2[0]) and len(Var_1[0]) == len(Var_3[0]) and len(Var_1[0]) == len(Var_4[0]):
        
        Met_ave = [Var_1[0],[0]*len(Var_1[0])]
        
        for i in range(2,len(Var_1)):
            
            Met_ave.append([0]*len(Var_1[0]))
            
            for j in range(len(Var_1[0])):
                ...
                if Var_1[i][j] != None:
                    ind = ind+1
                    Value = Value + Var_1[i][j] 
                if Var_2[i][j] != None:
                    ind = ind+1
                    Value = Value + Var_2[i][j] 
                if Var_3[i][j] != None:
                    ind = ind+1
                    Value = Value + Var_3[i][j] 
                if Var_4[i][j] != None:
                    ind = ind+1
                    Value = Value + Var_4[i][j]
                if ind==0 :
                    Met_ave[i][j] = None
                else: 
                    Met_ave[i][j] = Value/ind
                    Met_ave[1][j] = Met_ave[1][j] + ind/20
                    
    else:
        
        print('Erro vectors not the same size for averaging operation!')
